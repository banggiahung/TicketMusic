...
public class VnPayLibrary
{
    ...
    public PaymentResponseModel GetFullResponseData(IQueryCollection collection, string hashSecret)
    {
        ...
        UInt64 orderId;
        UInt64.TryParse(vnPay.GetResponseData("vnp_TxnRef"), out orderId);
        ...
        var vnp_Amount = !string.IsNullOrEmpty(amountString) ? (decimal.Parse(amountString) / 100) : 0;
        ...
        {
            { "00|00", "Confirm Success" },
            { "02|02", "Order already confirmed" },
            { "01", "Order not found" },
            { "97", "Invalid Checksum" },
            { "04", "Invalid amount" }
        };

            string key = $"{vnpResponseCode}|{vnp_TransactionStatus}";
            string message = responseMessages.ContainsKey(key) ? responseMessages[key] : "Order already confirmed";
